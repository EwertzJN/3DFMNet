...
from vision3d.modules.kpconv.helpers import calibrate_neighbors, generate_input_data


def odometry_kitti_kpconv_collate_fn(data_dicts, config, neighborhood_limits):
    new_data_dicts = []

    for data_dict in data_dicts:
        new_data_dict = {}

        new_data_dict['seq_id'] = data_dict['seq_id']
        new_data_dict['frame0'] = data_dict['frame0']
        new_data_dict['frame1'] = data_dict['frame1']
...
def get_dataloader(
    ...
    if neighborhood_limits is None:
        neighborhood_limits = calibrate_neighbors(dataset, config, collate_fn=odometry_kitti_kpconv_collate_fn)
    dataloader = torch.utils.data.DataLoader(
        ...
        sampler=sampler,
        collate_fn=partial(odometry_kitti_kpconv_collate_fn, config=config, neighborhood_limits=neighborhood_limits),
        drop_last=drop_last
    )
