...
from vision3d.utils.registration_utils import get_node_corr_indices_and_overlaps,to_o3d_pcd,to_array,to_tensor,compute_relative_rotation_error
...
error_path = ['/data1/huile/zhangliyuan/multi_instance/shapenet/ShapeNetCore.v2/04379243/e7580c72525b4bb1cc786970133d7717/models/model_normalized.obj',
            '/data1/huile/zhangliyuan/multi_instance/shapenet/ShapeNetCore.v2/02933112/cdc3762d2846133adc26ec30fe28341a/models/model_normalized.obj']
correct_path = ['/data1/huile/zhangliyuan/multi_instance/shapenet/error_path/04379243/model_normalized.obj', 
                '/data1/huile/zhangliyuan/multi_instance/shapenet/error_path/02933112/model_normalized.obj']
...
class Process_ROBISENCEDataset(torch.utils.data.Dataset):
    def __init__(self,
                 robi_root,
                 vox,
                 split,
                 ...
                 aligned=False,
                 rotated=False):
        super(Process_ROBISENCEDataset, self).__init__()
        self.vox_size=vox
        self.scan2cad_root = robi_root
        self.partition = split
        ...
        self.aligned = aligned
        self.rotated = rotated
        self.train_num = 46628 # 46628 2911*n+n-1 
        self.val_num = 0 # 218, 0 40805 
        self.test_num = 28617 # 438, 1598 22296 28617
        if self.partition == 'train':
    ...
    def __getitem__(self, id):
        ...
        if self.partition == 'train':
            # split_number = np.loadtxt(self.split_root+'count{:05}.txt'.format(frame_id // 2)).astype(np.int32)
            # choose_frame_id = np.random.choice(split_number)
            choose_frame_id = frame_id
        else:
            choose_frame_id = frame_id
        dataset=np.load(self.scan2cad_root+ 'data{:05d}.npz'.format(choose_frame_id))
        ...
        return points0,points1,feats0,feats1,Rts,correspondences, scene_name,sym,sym, points0,points1
...
class Scan2cadKPConvDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        self.process_scan2cad_root = process_scan2cad_root
        self.partition = split
        self.matching_radius = matching_radius
    ...
    def __getitem__(self, id):
        ...
        if cadroot in error_path:
            # print(cadroot)
            cadroot = correct_path[error_path.index(cadroot)]
            # print(cadroot)
        ...
        feats1 = np.ones((points1.shape[0], 1), dtype=np.float32)
        correspondences=[]
        savename = self.process_scan2cad_root + 'data{:05d}.npz'.format(frame_id)
        np.savez(savename, points0=points0, points1=points1, trans=Rts, frag_id1=frame_id, scene_name=id_scan, sym=frame_id)
...
def threedmatch_kpconv_collate_Roitr(list_data):
    data_dicts = []

    for points0, points1, feats0, feats1, transform, correspondences, scene_name, frag_id0, frag_id1 ,points0_normal,points1_normal in list_data:
        data_dict = {}
        ...
        #data_dict['correspondences'] = torch.from_numpy(correspondences)
        data_dict['correspondences'] =correspondences
        data_dict['features1'] = torch.from_numpy(feats1)
        data_dict['features0'] = torch.from_numpy(feats0)

        data_dict['points0'] = torch.from_numpy(points0)
        data_dict['points1'] = torch.from_numpy(points1)
        data_dict['points0_normal'] = torch.from_numpy(points0_normal)
        data_dict['points1_normal'] = torch.from_numpy(points1_normal)
        data_dicts.append(data_dict)
...
def get_dataloader_Roitr(
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        sampler=sampler,
        collate_fn=partial(
            threedmatch_kpconv_collate_Roitr
        ),
        drop_last=drop_last
    )
    return dataloader