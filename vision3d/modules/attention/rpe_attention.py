...
class RPEMultiHeadAttention(nn.Module):
    ...
    def forward(self, input_q, input_k, input_v, input_p, key_masks=None):
        ...
        #print(key_masks.shape[1]-torch.sum(key_masks,dim=1))
        attention_scores = F.softmax(attention_scores, dim=-1)
        #print(attention_scores_ori[attention_scores.isnan()])
...
class Mask_RPETransformer5_paper(nn.Module):
    def __init__(self, blocks, d_model, num_head, dropout=0.1, activation_fn='gelu'):
        super(Mask_RPETransformer5_paper, self).__init__()
        self.blocks = blocks
        layers = []
