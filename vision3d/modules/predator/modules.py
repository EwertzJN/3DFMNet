...
from ..geometry.functional import k_nearest_neighbors
...
class SelfAttention(nn.Module):
    ...
    def forward(self, coords, features):
        ...
        Output:
            feats:      [B, C, N]
        """
        idx = k_nearest_neighbors(coords, coords, self.k + 1)[:, :, 1:]

        B, C, N = features.size()

        x0 = features.unsqueeze(-1)  # [B, C, N, 1]

        x1 = get_graph_feature(idx, x0.squeeze(-1))
        x1 = F.leaky_relu(self.in1(self.conv1(x1)), negative_slope=0.2)
        x1 = x1.max(dim=-1, keepdim=True)[0]

        x2 = get_graph_feature(idx, x1.squeeze(-1))
