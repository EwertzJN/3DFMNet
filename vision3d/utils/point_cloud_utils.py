...
def get_node_nearest_centers_offset(ref_points, gt_instance_centers):
    # breakpoint()
    dist = pairwise_distance(ref_points, gt_instance_centers)
    min_dist, min_dists_indices = torch.min(dist, dim=1)
    center_offsets = gt_instance_centers.unsqueeze(0).repeat(ref_points.shape[0], 1, 1) - ref_points.unsqueeze(1).repeat(1, gt_instance_centers.shape[0], 1)
    gt_center_offsets = center_offsets[torch.arange(ref_points.shape[0]), min_dists_indices, :]
    gt_centers = gt_instance_centers.unsqueeze(0).repeat(ref_points.shape[0], 1, 1)[torch.arange(ref_points.shape[0]), min_dists_indices, :]
    return gt_center_offsets, gt_centers
...
def get_point_to_node(points, nodes, return_counts=False):
    ...
    if isinstance(points, torch.Tensor):
        ...
        if return_counts:
            ...
            node_sizes = torch.zeros(nodes.shape[0], dtype=torch.long).to(nodes.device)
...
def get_point_to_node_indices_and_masks(points, nodes, num_sample, return_counts=False):
    ...
    node_knn_indices = get_knn_indices(points, nodes, num_sample)  # (num_node, max_point)
    node_indices = torch.arange(nodes.shape[0]).to(nodes.device).unsqueeze(1).expand(-1, min(num_sample,points.shape[0]))
