...
from utils.dataset import ROBI_test_data_loader
...
def make_parser():
    ...
    parser.add_argument('--test_epoch', type=int, required=True, default=60)
    ...
    parser.add_argument('--detcenter_dir', type=str, default='/data1/local_userdata/zhangliyuan/log/NIPS24/baseline/robi/detection_exp0/detcenter')
...
def run_one_epoch(
    ...
    for i, data_dict in enumerate(data_loader):
        ...
        with torch.no_grad():                
            ...
            mean_instance_recall += pred_cluster_recall
            ...
            if engine.args.if_proposal:
                pred_centers = output_dict['pred_instance_centers']
                savename = os.path.join(engine.args.detcenter_dir, 'data{:05d}.npz'.format(i))
                save_pred_center = pred_centers.cpu().detach().numpy()
                np.savez(savename, pred_centers=save_pred_center)
            count += 1
            start += 1
        
    print('pred_cluster_instance_recall', 100*mean_instance_recall/count)
...
def main():
    ...
    config.detcenter_dir = args.detcenter_dir
    config.robi_root = args.robi_root
    ensure_dir(config.logs_dir)
    ensure_dir(config.detcenter_dir)
    ...
    with Engine(log_file=log_file, default_parser=parser, seed=config.seed) as engine:
        ...
        # data_loader = ROBI_test_data_loader(engine, config)
        ...
        snapshot = "./pretrain/robifocus.pth.tar"
